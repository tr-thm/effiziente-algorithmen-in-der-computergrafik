...
void Graphics::loop()
{
    ...
    static int fenceIndex = 0;
    fences[fenceIndex] = glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
    fenceIndex = (fenceIndex + 1) % 3;
    glFlush();
    glClientWaitSync(fences[fenceIndex], GL_SYNC_FLUSH_COMMANDS_BIT, 100000000);
