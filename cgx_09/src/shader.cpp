...
Shader::Shader(std::string vertexShaderFile, std::string fragmentShaderFile)
{
    ...
    this->vars = {
        ...
        glGetUniformLocation(shaderProgram, "AR"),
        glGetUniformLocation(shaderProgram, "NOM"),
        glGetUniformLocation(shaderProgram, "PMDG"),
        glGetUniformLocation(shaderProgram, "ViewportSize"),
        glGetUniformLocation(shaderProgram, "SunBrightness")
    };
...
void Shader::setTexture(int location, Texture *texture)
{
    ...
    else if (location == vars.Skybox)
    {
        ...
        glBindTexture(GL_TEXTURE_CUBE_MAP, ((CubeMap*)texture)->getId());
    }
    else if (location == vars.AR)
    {
        glUniform1i(location, 4);
        glActiveTexture(GL_TEXTURE4);
        glBindTexture(GL_TEXTURE_2D, texture->getId());
    }
    else if (location == vars.NOM)
    {
        glUniform1i(location, 5);
        glActiveTexture(GL_TEXTURE5);
        glBindTexture(GL_TEXTURE_2D, texture->getId());
    }
    else if (location == vars.PMDG)
    {
