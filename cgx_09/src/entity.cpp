...
bool Entity::collidesWith(Entity *other, bool exact)
{
    if (this->collider == nullptr || other->collider == nullptr) return false;
    return this->collider->collidesWith(other->collider, exact);
}
...
void Entity::update(const double time)
{
    ...
    {
        this->collider->update(position, rotation, scale);
    }
}
